...
Original file is located at
    https://colab.research.google.com/drive/12SjgCK4RYvdFhZOKIHS7pgvh43P8HAL5
...
for participant_id in range (1,21):
  for x in ['low','high']:
    fat_block = df[(df['participant'] == participant_id) & (df['block'] == 'health')& (df['fat'] == x)]

    valid_ratings = fat_block['rating'].dropna()
...
for participant in range(1, 3):
    participant_block = data[data['participant'] == participant]
    left_choices  = participant_block[participant_block['choice'] == 1]['money_left']
    right_choices = participant_block[participant_block['choice'] == 2]['money_right']

    average_earnings = pd.concat([left_choices, right_choices]).dropna().mean()
    print(f"Participant {participant}'s average earnings: ${average_earnings:.2f}")

for x in [1, 0]:
  trial_block = data[(data['participant'] == participant) & (data['delayed_opt_chose'] == x)]
  number_trials = len(trial_block)
  choice_decision = "delayed" if x == 1 else "sooner"
  print(f"Participant {participant} chose the {choice_decision} option {number_trials} times")
